'use client';

import { useState } from 'react';
import { Capacitor } from '@capacitor/core';
import { Browser } from '@capacitor/browser';
import { toast } from 'sonner';
import { googleSignInAction } from '@/app/(auth)/actions';
import { createClient } from '@/infrastructure/supabase/client';
import { Button } from '@/presentation/components/ui/button';

const NATIVE_REDIRECT_URL = 'com.jorgehernandez.finance://auth/callback';

/**
 * En nativo no se puede usar el <form action={googleSignInAction}> normal:
 * ese Server Action navega dentro del WebView embebido, y Google bloquea el
 * login OAuth ahí (ver NativeAuthBridge.tsx). En su lugar se pide la URL de
 * Google client-side con skipBrowserRedirect y se abre en el navegador del
 * sistema — NativeAuthBridge recoge el regreso por deep link.
 */
async function signInWithGoogleNative() {
  const supabase = createClient();
  const { data, error } = await supabase.auth.signInWithOAuth({
    provider: 'google',
    options: { redirectTo: NATIVE_REDIRECT_URL, skipBrowserRedirect: true },
  });

  if (error || !data.url) {
    toast.error('No se pudo iniciar el login con Google.');
    return;
  }

  await Browser.open({ url: data.url });
}

export function GoogleButton() {
  const [isNativeLoading, setIsNativeLoading] = useState(false);
  const isNative = Capacitor.isNativePlatform();

  if (isNative) {
    return (
      <Button
        type="button"
        variant="outline"
        className="w-full"
        disabled={isNativeLoading}
        onClick={async () => {
          setIsNativeLoading(true);
          try {
            await signInWithGoogleNative();
          } finally {
            setIsNativeLoading(false);
          }
        }}
      >
        <GoogleIcon />
        Continuar con Google
      </Button>
    );
  }

  return (
    <form action={googleSignInAction}>
      <Button type="submit" variant="outline" className="w-full">
        <GoogleIcon />
        Continuar con Google
      </Button>
    </form>
  );
}

function GoogleIcon() {
  return (
    <svg viewBox="0 0 24 24" className="size-4">
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.99.66-2.25 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.85A11 11 0 0 0 12 23z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.09A6.6 6.6 0 0 1 5.5 12c0-.73.12-1.43.34-2.09V7.06H2.18A11 11 0 0 0 1 12c0 1.77.43 3.45 1.18 4.94z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1A11 11 0 0 0 2.18 7.06l3.66 2.85C6.71 7.31 9.14 5.38 12 5.38z"
      />
    </svg>
  );
}
